...
namespace DevotedLlama.CircleGenerator.Tests {
    public class DashCircleTest {

        CircleTemplate _circle;
        Mesh _mesh = new Mesh();


        [TestCaseSource(typeof(DashCircleTestData), nameof(DashCircleTestData.VertexPositions))]
        public Vector3[] TestVertexPositions(CircleData CD, StrokeData SD) {
            _circle = new DashCircle(_mesh, CD);
            _circle.Accept(new CircleStrokeVisitor(SD));
            _mesh = _circle.GetMesh();
            return _mesh.vertices;
        }

        [TestCaseSource(typeof(DashCircleTestData), nameof(DashCircleTestData.Triangles))]
    ...
    public class DashCircleTestData {
        ...
        static int[] GetTriangles() {
            ...
            if (CD.Completion > 0) {
                ...
                for (int ti = 0, vi = 0, x = 0; x < CD.PolygonQuantity; x++, ti += triangleIncrement, vi += 2) {
                    triangles[ti] = vi;
                    triangles[ti + 1] = triangles[ti + 4] = vi + 1;
                    triangles[ti + 2] = triangles[ti + 3] = vi + (CD.PolygonQuantity * 2);
                    triangles[ti + 5] = vi + (CD.PolygonQuantity * 2) + 1;
                }
            }
            return triangles;
        }
